...
export const GENERATED_TOOL_CATALOG = [
  ...
  },
  {
    tool: {
      ...
      description:
        'Array-of-Bytes scan with wildcard support. Search for byte patterns like "48 8B ?? ?? 00 00" across readable memory. Accepts hex bytes (00-FF, optional 0x prefix) and "??" wildcards. Case insensitive.',
      inputSchema: {
        type: 'object',
        properties: {
          pid: {
    ...
    tool: {
      ...
      inputSchema: {
        ...
        properties: {
          action: {
            type: 'string',
            enum: ['scan', 'validate', 'resolve', 'export'],
  ...
  },
  {
    tool: {
      ...
      description:
        'Enumerate memory regions in a target process. Cross-platform: Windows (VirtualQueryEx), macOS (mach_vm_region), Linux (/proc/pid/maps). Returns base address, size, protection (r/w/x/rw/rx/rwx), state, type (image/mapped/private), and module name (if module-backed).',
      inputSchema: {
        type: 'object',
        properties: {
          pid: {
  ...
  },
  {
    tool: {
      name: 'pcapng_read',
      description:
        'Read a PCAPNG (pcap-ng) capture file and return structured Section/Interface/Packet blocks. Supports Section Header, Interface Description, Enhanced/Simple Packet, Name Resolution, and Interface Statistics blocks; unknown block types are surfaced as warnings.',
      inputSchema: {
        type: 'object',
        properties: {
          path: {
  ...
  },
  {
    tool: {
      name: 'v8_deopt_trace',
      description:
        'Trace V8 deoptimization events during a capture window. Enables %TraceDeoptimizations via natives syntax and captures deopt events (function name, reason, bailout position). Requires V8 natives syntax. Falls back gracefully when unavailable.',
      inputSchema: {
        type: 'object',
        properties: {
          durationMs: {
  ...
  },
  {
    tool: {
      ...
      description:
        'Inspect TurboFan compilation state for functions in a script. Reports optimization tier (interpreted/maglev/turbofan). Supports actions: inspect (default), optimize (%OptimizeFunctionOnNextCall), deoptimize (%DeoptimizeFunction). Requires V8 natives syntax.',
      inputSchema: {
        type: 'object',
        properties: {
          scriptId: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          stepResults: {
            ...
            additionalProperties: true,
            description:
              'Optional map of stepId → result to evaluate the predicate against. When omitted, fetched from the last successful workflow run for workflowId.',
          },
          workflowId: {
            type: 'string',
            description:
              'Workflow or macro id used to look up the last successful run stepResults when stepResults is not provided.',
          },
        },
  ...
  },
  {
    tool: {
      ...
      description:
        'Inspect the global workflow run store: list recent run_extension_workflow / run_macro runs, get a run entry by runId, or fetch the last successful full result (stepResults, spans, metrics) for a workflow or macro id.',
      inputSchema: {
        type: 'object',
        properties: {
          action: {
            ...
            description:
              'Inspection action. "list" returns recent runs (optionally filtered by workflowId); "get" returns one run entry by runId; "lastSuccess" returns the last ok result for a workflowId.',
            default: 'list',
          },
          runId: {
            type: 'string',
